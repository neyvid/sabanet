                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        @foreach($areacodes as $areacode)
                            <tr>
                                ...
                                <td>{{$areacode->code}}</td>
                                <td>{{$areacode->state->name}}</td>
                                <td>{{$areacode->city->name}}</td>
                                <td>{{$areacode->telecomcenter->name}}</td>
                                <td>{{ $areacode->present()->showOpratorName }}

                                </td>
                                <td>
                                    <a href="{{route('profile.areacode.edit').'?item='.$areacode->id}}"><i
                                            data-toggle="tooltip" data-placement="top"
                                            class="fa fa-pencil-square fa-2x editColor"
                                            title="ویرایش"></i></a>
                                    <a onclick="confirmDelete(event,this)"
                                       href="{{route('profile.areacode.delete').'?item='.$areacode->id}}"><i
                                            data-toggle="tooltip" data-placement="top"
